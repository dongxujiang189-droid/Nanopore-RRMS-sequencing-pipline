...
for chrom in genes["chrom"].unique():
    ...
    offset += chrom_len + 1_000_000  # 1 Mb spacer between chromosomes
...
def compute_gene_mods(df_mod, df_genes):
    ...
    for _, g in df_genes.iterrows():
        sub = df_mod[(df_mod["chrom"] == g["chrom"]) &
                     (df_mod.index >= g["start"]) & (df_mod.index <= g["end"])]
...
for bc in samples_files.keys():
    ...
    df_gene_mod.fillna(0.01, inplace=True)  # avoid log(0)
...
plt.tight_layout()
out_fig = os.path.join(data_dir, "modkit_genome_wide_gene_modifications.png")
...
for bc in samples_files.keys():
    sub_mod = mod_all[mod_all["sample"] == bc]
    df_gene_mod = compute_gene_mods(sub_mod, genes)
    df_gene_mod["sample"] = bc
    gene_summary.append(df_gene_mod)
gene_summary = pd.concat(gene_summary, ignore_index=True)

out_tsv = os.path.join(data_dir, "modkit_genome_wide_gene_summary.tsv")
gene_summary.to_csv(out_tsv, sep="\t", index=False)
