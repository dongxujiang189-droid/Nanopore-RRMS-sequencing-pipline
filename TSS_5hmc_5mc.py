...
for sample_file in sample_files:
    ...
    # Rename #chrom to chrom if needed
    if '#chrom' in meth_df.columns:
        meth_df = meth_df.rename(columns={'#chrom': 'chrom'})

    required_cols = {'chrom', 'start', 'end', 'percent_m', 'percent_h'}
    missing = required_cols - set(meth_df.columns)
    if missing:
        raise ValueError(f"Missing expected columns {missing} in {sample_file}")

    # Filter data within ±2kb of any TSS
    tss_regions = []
    for _, tss_row in tqdm(tss_df.iterrows(), total=len(tss_df), desc=f"Filtering {sample_name}"):
        region = meth_df[
    ...
    meth_mean = [np.mean(meth_profile[bc]) if bc in meth_profile else np.nan for bc in bin_centers]
    hmeth_mean = [np.mean(hmeth_profile[bc]) if bc in hmeth_profile else np.nan for bc in bin_centers]
...
print(f"  - tss_methylation_profiles.png (combined plot)")
